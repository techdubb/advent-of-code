...
class OperatorPacket:
    ...
    def value(self):
        if self.type_id in (5,6,7):
            l = self.inner_packet[0]
            r = self.inner_packet[1]

        if self.type_id == 0:
            return sum([p.value() for p in self.inner_packet])
        elif self.type_id == 1:
            return reduce(mul, [p.value() for p in self.inner_packet])
        elif self.type_id == 2:
            return min([p.value() for p in self.inner_packet])
        elif self.type_id == 3:
            return max([p.value() for p in self.inner_packet])
        elif self.type_id == 5:
